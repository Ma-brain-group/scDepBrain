################ Stratified analysis visualize the result of scDRS #####################
rm(list = ls())
library(Seurat)
library(ggplot2)
library(ggvenn) 
...
setwd("D:/Project/SingleCell_MDD/GWAS_combine_singlecell_analysis/6_分层分析/Sex/scDRS/") 
list.files(getwd())
...
for (i in 1:length(file)) {
  print(file[i])
  scDRS<-fread(paste0("./",file[i],"/MDD.scdrs_ct.anno"),header = T)
  #### get the information 
  scDRS<-scDRS[,c(1,4)]
  ...
  #### plot
  p<-ggdotchart(scDRS, x = "cell_type", y = "-log10(P)",
                color = "cell_type",                               
  ...
  ggsave(paste0("./",file[i],"/",file[i],"_scDRS.pdf"),p,width = 4.5,height = 3)
  scDRS<-NULL
}
