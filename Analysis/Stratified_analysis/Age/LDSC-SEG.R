################ Stratified analysis visualize the result of LDSC #####################
rm(list = ls())
library(Seurat)
library(ggplot2)
library(ggvenn) 
...
setwd("D:/Project/SingleCell_MDD/GWAS_combine_singlecell_analysis/6_分层分析/Sex/LDSC/") 
list.files(getwd())
...
for (i in 1:length(file)) {
  print(file[i])
  ldsc<-read.table(file[i],header = T)
  #### get the information 
  ldsc<-ldsc[,c(1,4)]
  ...
  #### plot
  p<-ggdotchart(ldsc, x = "cell_type", y = "-log10(P)",
                color = "cell_type",                               
  ...
  ggsave(paste0(file[i],"_ldsc.pdf"),p,width = 4.5,height = 3)
}
