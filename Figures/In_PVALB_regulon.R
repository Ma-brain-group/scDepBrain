##################### In_PVALB regulon plot ####################
rm(list = ls())
library(ggplot2)
library(dplyr)
library(tidyr)
setwd("D:/Project/SingleCell_MDD/Figure/Figure5/pyscenic_In_PVALB/")
...
###################### save the result
write.csv(regulon1,file = "In_PVALB_TF_target_genes.csv",quote = F)
...
pdf("In_PVALB_TF_target_genes.pdf",height = 4.5,width = 5.5)
...
write.csv(regulon2,file = "In_PVALB_average_TF_target_genes.csv",quote = F)
...
pdf("In_PVALB_average_TF_target_genes.pdf",height = 4.5,width = 5.5)
ggplot(regulon2,aes(X,Average_Gene_Count))+
  ...
  theme_bw()+
  xlab(" ")+
  ylab("The average target gene numbers")+
  theme(axis.text.x = element_text(size = 10,color = 'black',vjust = 0.5,hjust = 1,angle = 90), 
        axis.text.y = element_text(size = 10,color = 'black'), 
        legend.text = element_text(size = 10,color = 'black'), 
        legend.title = element_text(size = 10,color = 'black'), 
